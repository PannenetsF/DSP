...
理想时域采样： \[x_{s}(t)=x_{c}(t) * s(t)=\sum_{-\infty}^{+\infty} x_{c}(n T) \delta(\mathrm{t}-n T)\]

其中：\[s(t) = \sum_{-\infty}^\infty \delta(t - n T)\] 

频域表示为： \[
    \delta_{T_{1}}(t)=\sum_{-\infty}^{\infty} \delta\left(t-n T_{1}\right) \rightarrow \frac{2\pi}{T_1} \sum_{-\infty}^{\infty} \delta\left(\omega-n \omega_{1}\right)
    \]

\[\begin{aligned}
    X_s(\Omega) &= \frac{1}{2\pi} X_c (\Omega) S(\Omega) \\ 
    &= \frac{1}{T} X_c(\Omega) \otimes \sum_{-\infty}^{+\infty} \delta(\Omega - n \Omega_0 ) \\ 
    &= \frac{1}{T} \sum_{-\infty}^\infty X_c(\Omega - n \Omega_0) 
\end{aligned}
\]
...
=\int_{-\infty}^{+\infty}\left[\sum_{n=-\infty}^{+\infty} x_{c}(n T) \delta(t-n T)\right] e^{-j \Omega t} d t \\
=\sum_{n=-\infty}^{+\infty} \int_{-\infty}^{+\infty} x_{c}(n T) \delta(t-n T) e^{-j \Omega t} d t \\
=\sum_{n=-\infty}^{+\infty} x_{c}(n T) e^{-j(\Omega T) n}
\end{array}
\]
...
\[\begin{aligned}
    ...
    &=  \sum_{-\infty}^\infty x_c(n T) e^{-j\omega n} 
\end{aligned}\]
...
=\sum_{-\infty}^{+\infty} x(n) \delta(t-n T) \otimes h_{r}(t) \\
=\sum_{-\infty}^{+\infty} x(n) h_{r}(t-n T) \\
=\sum_{-\infty}^{+\infty} x(n) \frac{\sin (\pi(t-n T) / T)}{\pi(t-n T) / T}
\end{array}
\]
...
\[\delta_{D}(n)=\frac{1}{D} \sum_{i=0}^{D-1} e^{j \frac{2 \pi}{D} n i}=\left\{\begin{array}{ll}
...
\[
\begin{array}{l}
x(n) \Leftrightarrow X s(\Omega)=\frac{1}{T} \sum_{k=-\infty}^{+\infty} X c\left(\Omega-k \Omega_{0}\right) \quad \Leftrightarrow X(\omega)=\frac{1}{T} \sum_{k=-\infty}^{+\infty} X c\left(\frac{\omega}{T}-k \frac{2 \pi}{T}\right) \\
...
\[X_D(\omega) = \frac{1}{D} \sum_{i = 0 }^{D-1} X(\frac{\omega - 2 \pi i}{D})\] 
...
=\sum_{m=-\infty}^{+\infty} X_{I}(m) e^{-j \omega m} \\
=\sum_{k=-\infty}^{+\infty} X_{I}(k I) e^{-j \omega I k} \\
=\sum_{k=-\infty}^{+\infty} X(k) e^{-j \omega I k}= X(e^{j\omega I})
\end{array}
\]
...
滤波器只需满足在最小频带工作：同时补偿进行内插的增益 \(I\) 。

\[
H\left(e^{j \omega}\right)=\left\{\begin{array}{ll}
I & |\omega| \leq \min \left(\frac{\pi}{I}, \frac{\pi}{D}\right) \\
0 & \text { else }
\end{array}\right.
\]
